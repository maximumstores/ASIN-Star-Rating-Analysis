    ...
    try:
        ...
        if is_empty_sheet:
            ...
            while current_row < len(all_values):
                ...
                if row[2]:  # Если есть ASIN в столбце C
                    ...
                    # Сохраняем позиции всех 12 строк для каждого ASIN
                    existing_asins[asin] = {
                        ...
                        'rows': {
                            ...
                            '2': current_row + 5,
                            '1': current_row + 6,
                            'Total Ratings: 5': current_row + 7,
                            'Total Ratings: 4': current_row + 8,
                            'Total Ratings: 3': current_row + 9,
                            'Total Ratings: 2': current_row + 10,
                            'Total Ratings: 1': current_row + 11
                        }
                    }
                    current_row += 12  # Переходим к следующему блоку из 12 строк
                else:
                    current_row += 1
        ...
        for product in data:
            ...
            parameters = [
                ...
                ("Total Ratings: 5", product.get('five_star', {}).get('Total Ratings', '0')),
                ("Total Ratings: 4", product.get('four_star', {}).get('Total Ratings', '0')),
                ("Total Ratings: 3", product.get('three_star', {}).get('Total Ratings', '0')),
                ("Total Ratings: 2", product.get('two_star', {}).get('Total Ratings', '0')),
                ("Total Ratings: 1", product.get('one_star', {}).get('Total Ratings', '0'))
            ]
